...
public class subjectTable2 extends Fragment implements View.OnClickListener, updateData{
    ...
    private void GetData(){
        SJArray.clear();
        SJArray.addAll(database.getAll_tue_Subjects());
        adapter.notifyDataSetChanged();
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        if (requestCode == RECOGNIZER_RESULT && resultCode == getActivity().RESULT_OK){
            ...
            database.insert(2, matches.get(0));
            GetData();
        }
    ...
    private void AddDialog(){
        ...
        addBttn.setOnClickListener(v -> {
            ...
            if (name.equals("")){
                database.insert(2, "null");
                dialog.dismiss();
                GetData();
            }else {
                database.insert(2, name);
                dialog.dismiss();
                GetData();
            }
        });
    ...
    private void RenameDialog(String name, final int id){
        ...
        confirmBttn.setOnClickListener(v -> {
            ...
            database.updateName(2, newName, id);
            dialog.dismiss();
            GetData();
        });
