...
public class subjectTable6 extends Fragment implements View.OnClickListener, updateData{
    ...
    private void GetData(){
        SJArray.clear();
        SJArray.addAll(database.getAll_sat_Subjects());
        adapter.notifyDataSetChanged();
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        if (requestCode == RECOGNIZER_RESULT && resultCode == getActivity().RESULT_OK){
            ...
            database.insert(6, matches.get(0).toString());
    ...
    private void AddDialog(){
        ...
        addBttn.setOnClickListener(v -> {
            ...
            if (name.equals("")){
                database.insert(6, "null");
                dialog.dismiss();
                GetData();
            }else {
                database.insert(6, name);
                dialog.dismiss();
                GetData();
            }
        });
    ...
    private void RenameDialog(String name, final int id){
        ...
        confirmBttn.setOnClickListener(v -> {
            ...
            database.updateName(6, newName, id);
            dialog.dismiss();
            GetData();
        });
